    ...
    if (trk_parameters.dump)
        {
            ...
            tmp_E = reinterpret_cast<float>(d_E_accu);  //removed abs()
            tmp_P = reinterpret_cast<float>(d_P_accu);
            tmp_L = reinterpret_cast<float>(d_L_accu);
            if (integrating)
                {
...
int dll_pll_veml_tracking::save_matfile()
{
    ...
    int epoch_size_bytes = sizeof(unsigned long int) + sizeof(double) * number_of_double_vars +
                           ...
                           sizeof(std::chrono::high_resolution_clock::time_point) * number_of_time_vars +
